...
import { Hash, Volume2, User, Users, Plus, X, Check, MessageSquare, Download } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentChannel, setCurrentChannel, isMobileOpen, closeMobile }) => {
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-64 transform transition-transform duration-300 ease-in-out ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 flex flex-col panel md:ml-3 md:my-4 md:rounded-2xl`}>
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-6 custom-scrollbar">
          ...
          <div className="px-2">
            <div className="px-2 text-[11px] font-bold text-zinc-400 uppercase mb-1 flex justify-between items-center group cursor-pointer hover:text-zinc-300">
              <span>Groups</span>
              <button onClick={(e) => { e.stopPropagation(); alert('Groups coming soon!'); }} className="text-zinc-400 hover:text-white">
                <Plus className="w-3 h-3" />
              </button>
            </div>
        ...
        {/* User User Bar */}
        <div className="h-[56px] bg-slate-900/80 flex items-center px-2 border-t border-white/5 rounded-b-2xl md:rounded-b-2xl">
          <div 
